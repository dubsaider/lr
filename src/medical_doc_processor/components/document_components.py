...
class DocumentComponents:
    ...
    def __init__(self, width: int, height: int, margin: int, square_size: int,
                 marker_size: int, language: str = "ru"):
        self.width = width
        self.height = height
        self.margin = margin
    ...
    def draw_time_fields_side_by_side(self, img: np.ndarray,
                                       squares_layout: List[Tuple[Tuple[int, int], str]],
                                       times: Dict[str, str]):
        ...
        # Формируем текст
        time_left_text = (f"{self.text_renderer.get_text('time')} "
                          f"{self.text_renderer.get_text('left')} ___________ "
                          f"{self.text_renderer.get_text('sec')}")

        # Вычисляем ширину текста для центрирования
        (text_w, text_h), _ = cv2.getTextSize(
            time_left_text, cv2.FONT_HERSHEY_SIMPLEX, 0.8 * base_font_scale, 2)
        ...
        # Формируем текст
        time_right_text = (f"{self.text_renderer.get_text('time')} "
                           f"{self.text_renderer.get_text('right')} ___________ "
                           f"{self.text_renderer.get_text('sec')}")

        # Вычисляем ширину текста для центрирования
        (text_w, text_h), _ = cv2.getTextSize(
            time_right_text, cv2.FONT_HERSHEY_SIMPLEX, 0.8 * base_font_scale, 2)
